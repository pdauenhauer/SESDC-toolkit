...
cors_settings = options.CorsOptions(cors_origins=[r"http://127\.0\.0\.1:4000", r"http://127\.0\.0\.1:5000"], cors_methods=["get", "post"],)
...
def run_simulation(req: https_fn.Request) -> https_fn.Response:  
    ...
    blob_path = f"{userId}/{projectId}/test_app_wind.csv"
    ...
    try:
        ...
        print(userId)
        print(projectId)
        solar_plot_path = generate_power_graph(time_points, solar_power_subset, userId, projectId)
        ...
        wind_plot_path = plot_wind_energy(time_points, wind_energy_subset, userId, projectId)
        ...
        diesel_plot_path = plot_generic(time_points, diesel_energy_subset, "Diesel Energy", "Diesel Energy Generation", "diesel_plot", userId, projectId)
        ...
        return https_fn.Response(f"Graph generated at {net_energy_plot_path}")
    except Exception as e:
        return https_fn.Response(f"Error processing file")
...
def fetch_solar_data(latitude, longitude, userId, projectId, api_key="TC1RGLjNIwUJrOGGyPvg4Fgq182jznjbbeIfZT5f", year="2022", interval="30"):
    ...
    if response.status_code == 200:
        ...
        run_simulation(userId, projectId)
    else:

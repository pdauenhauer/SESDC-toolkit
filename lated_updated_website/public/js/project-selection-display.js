...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('manualInputMode').addEventListener('click', function() {
        this.classList.add('active');
        document.getElementById('csvInputMode').classList.remove('active');
        document.getElementById('manualInputButtons').style.display = 'flex';
        document.getElementById('csvInputButtons').style.display = 'none';
        document.getElementById('save-csv').style.display = 'none';
        document.getElementById('save-manual-inputs').style.display = 'block';
        // Hide any open input sections
        document.querySelectorAll('.form-group').forEach(section => {
            section.style.display = 'none';
        });
    });
    
    document.getElementById('csvInputMode').addEventListener('click', function() {
        this.classList.add('active');
        document.getElementById('manualInputMode').classList.remove('active');
        document.getElementById('manualInputButtons').style.display = 'none';
        document.getElementById('csvInputButtons').style.display = 'flex';
        document.getElementById('save-manual-inputs').style.display = 'none';
        document.getElementById('save-csv').style.display = 'block';
        // Hide all manual input sections
        document.querySelectorAll('.form-group').forEach(section => {
            section.style.display = 'none';
        });
    });
    ...
    document.getElementById('resetBtn').addEventListener('click', async function() {
        ...
        try {
            const isManualMode = document.getElementById('manualInputMode').classList.contains('active');
            ...
            if (isManualMode) {
                ...
                document.getElementById('csvInputMode').style.display = 'block';
                ...
                document.getElementById('manualInputMode').style.display = 'block';
                document.getElementById('resetBtn').style.display = 'none';
            }
    
        } catch (error) {
            console.error('Error resetting project:', error);
            alert('Error resetting project. Please try again.');
        }
    });
    ...
    document.querySelector('.save-submit-btn').addEventListener('click', async (e) => {
        ...
        if (data.uploadedCsv) {
            ...
            try {            
                ...
                // /run_simulation
                // https://us-central1-sesdc-function-test.cloudfunctions.net/csv_processing
                // https://us-central1-sesdc-function-test.cloudfunctions.net/run_simulation
                // http://127.0.0.1:5001/sesdc-function-test/us-central1/run_simulation
                // https://csv-processing-2e75hqar4q-uc.a.run.app
                const response = await fetch('https://csv-processing-2e75hqar4q-uc.a.run.app', {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify ({
                        ...
                        // Battery parameters
                        usingBattery: batteryInputs.usingBattery,
...
/*
document.querySelectorAll('.csv-input-buttons button').forEach(button => {
    button.addEventListener('click', function(e) {
        ...
        document.querySelectorAll('.csv-input-buttons button').forEach(btn => {
            btn.style.backgroundColor = '';
        })

        if (this.id === 'addCsvBtn') {
...
document.querySelectorAll('#csvInputButtons button').forEach(button => {
    button.addEventListener('click', function (e) {
        e.preventDefault();
        const inputsId = this.id.replace('Btn', '');
        const inputs = document.getElementById(inputsId);
        ...
        document.querySelectorAll('#csvInputButtons button').forEach(btn => {
            btn.style.backgroundColor = '';
        });
...
document.getElementById('save-csv').addEventListener('click', async (e) => {
    ...
    try {
        ...
        document.getElementById('manualInputMode').style.display = 'none';
...
document.getElementById('save-manual-inputs').addEventListener('click', async (e) => {
    ...
    try {
        ...
        document.getElementById('csvInputMode').style.display = 'none';
        document.getElementById('resetBtn').style.display = 'block';

...
function UserGuide() {
  useEffect(() => {
    ...
    const navLinks = document.querySelectorAll<HTMLAnchorElement>('.sidebar .nav-link')

    const onScroll = () => {
      let currentId = ''
      sections.forEach(section => {
        const sectionTop = (section as HTMLElement).offsetTop - 120
        if (window.scrollY >= sectionTop) currentId = (section as HTMLElement).id
      })

      navLinks.forEach(link => {
        link.classList.remove('active-section', 'active-subsection')
        if (link.classList.contains('has-sub')) {
          link.classList.remove('open')
          const submenu = link.nextElementSibling as HTMLElement | null
          if (submenu && submenu.classList.contains('sub-links')) submenu.style.display = 'none'
        }
      })

      navLinks.forEach(link => {
        if (link.getAttribute('href') === '#' + currentId) {
          const parentSubLinks = link.closest('.sub-links') as HTMLElement | null
          if (parentSubLinks) {
            link.classList.add('active-subsection')
            const parent = parentSubLinks.previousElementSibling as HTMLElement | null
            if (parent && parent.classList.contains('has-sub')) {
              parent.classList.add('active-section', 'open')
              parentSubLinks.style.display = 'flex'
            }
          } else {
    ...
    // Accordion behavior for .has-sub links
    const togglers = document.querySelectorAll<HTMLAnchorElement>('.sidebar .nav-link.has-sub')
    const onToggle = (e: Event) => {
      const link = e.currentTarget as HTMLAnchorElement
      const submenu = link.nextElementSibling as HTMLElement | null
      if (submenu && submenu.classList.contains('sub-links')) {
        e.preventDefault()
        document.querySelectorAll('.nav-link.has-sub.open').forEach(other => {
          if (other !== link) {
            other.classList.remove('open')
            const otherMenu = (other as HTMLElement).nextElementSibling as HTMLElement | null
            if (otherMenu && otherMenu.classList.contains('sub-links')) otherMenu.style.display = 'none'
          }
        })
    ...
    togglers.forEach(t => t.addEventListener('click', onToggle))

    return () => {
      window.removeEventListener('scroll', onScroll)
      togglers.forEach(t => t.removeEventListener('click', onToggle))
    }
  }, [])
    ...
    <div>
      ...
      <div class="sidebar" id="sidebar">
        ...
        <div class="sub-links">
          ...
          <a href="#section1f" class="nav-link">Mobile/Desktop Compatibility</a>
        </div>

        <a href="#section2" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Getting Started</a>
        <div class="sub-links">
          <a href="#section2a" class="nav-link">Creating an Account & Logging in</a>
          <a href="#section2b" class="nav-link">Navigating Dashboard</a>
          <a href="#section2c" class="nav-link">Starting a New Project</a>
        </div>

        <a href="#section3" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Configuring a Microgrid</a>
        <div class="sub-links">
          <a href="#section3a" class="nav-link">Defining energy loads</a>
          <a href="#section3b" class="nav-link">Selecting energy sources</a>
          <a href="#section3c" class="nav-link">Location and environmental data</a>
          <a href="#section3d" class="nav-link">Simulation settings</a>
        </div>

        <a href="#section4" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Simulation</a>
        <div class="sub-links">
          <a href="#section4a" class="nav-link">How to start</a>
          <a href="#section4b" class="nav-link">Returning after changes</a>
        </div>

        <a href="#section5" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Analysis</a>
        <div class="sub-links">
          <a href="#section5a" class="nav-link">Visual outputs</a>
          <a href="#section5b" class="nav-link">Energy Balance</a>
          <a href="#section5c" class="nav-link">Reliability metrics</a>
        </div>

        <a href="#section6" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Cost & Economic Analysis</a>

        <a href="#section7" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Saving and Exporting</a>
        <div class="sub-links">
          <a href="#section7a" class="nav-link">Saving project progress</a>
          <a href="#section7b" class="nav-link">Exporting to PDF/CSV</a>
        </div>

        <a href="#section8" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Example Project</a>

        <a href="#section9" class="nav-link has-sub"><i class="bx bx-chevron-right arrow" /> Troubleshooting & FAQ</a>
        <div class="sub-links">
          <a href="#section9a" class="nav-link">Common errors</a>
          <a href="#section9b" class="nav-link">Reporting bugs or feedback</a>
          <a href="#section9c" class="nav-link">Contact/support link</a>
        </div>
      </div>
      ...
      <div class="main-content">
        ...
        <h3 id="section3d">Simulation settings (e.g., load growth assumptions)</h3>
        ...
        <p>Stay tuned for a step-by-step walkthroughs</p>
